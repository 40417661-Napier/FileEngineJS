...
{ // doesn't need to be passed anything as all vars / struct are in the .h file
    ...
    if (mainStruct.inputFlag == 0)
    {
        ...
        fgets(mainStruct.iArgVal, 1024, stdin);
        removeNewLine(mainStruct.iArgVal);
    }
    ...
    else
    {
        ...
        for (int i = 0; i < strlen(mainStruct.k_ArgVal); i++)
        {

            if (mainStruct.k_ArgVal[i] == '1' || mainStruct.k_ArgVal[i] == '0')
            {
                //do nothing, all is supposed to be
            }
            else
            {
                // printf("Invalid character detected, using 101 default key!\n");
                mainStruct.keyFailFlag = 1;
            }
        }
        ...
        else
        {
            ...
            if (mainStruct.key > 1111111 || mainStruct.key < 1)
            {
